...
- CI 工作流复核：`quality.yml:web` 和 `docker-image.yml:quality` 的 setup-node pnpm cache 均覆盖 `web/pnpm-lock.yaml`、`docs/pnpm-lock.yaml`，并在 E2E 前执行 `pnpm --dir ../docs install --frozen-lockfile`。`cd web && PATH=/opt/homebrew/bin:$PATH pnpm exec vitest run scripts/release-workflow-contract.test.mjs` 的 RED 为 exit 1（两处仅缓存 web lockfile），GREEN 为 exit 0、9 项通过；`pnpm --dir ../docs install --frozen-lockfile` 后三视口文档品牌 E2E 为 exit 0、6 项通过。

...
- [Agent/计费] 修复 Planner 已成功返回计划后，HTTP 流清理被误判为用户取消或较早终态重放因并发补充 Token 用量证据而触发结算冲突的问题；无法分类的下游清理交回业务调用方收尾，已结算的同身份取消记录可由有效计划安全恢复且不重复扣费，真实冲突仍保留原始类型与 HTTP 状态。生成运维同时展示规划结算的完整错误文本。
- [Agent] 修复越南语“创建视频脚本”被误判为直接生成视频：明确的脚本撰写请求只允许文本产物，服务端会在子任务派发前拒绝违反该意图的媒体计划；明确“从脚本创建视频”或同时要求脚本与视频时仍保留媒体生成。
- [网站设置] 后台站点设置新增企业名称、客服电话、客服邮箱与联系地址；保存后仅首页和 6 个产品落地页的页脚公开展示非空字段，三语标签随页面语言变化，电话和邮箱分别使用 `tel:`、`mailto:` 链接，清空字段会立即移除对应内容。
- [多语言] 公开 SEO 入口现按“显式语言路径 → 已保存选择 → 浏览器 `Accept-Language` → 越南语”在服务端协商语言：英语和中文设备分别临时跳转到 `/en`、`/zh-cn`，越南语或其他语言保持无前缀越南语入口；手动选择使用同一枚一年期 Cookie，旧 `/vi/*` canonical 跳转会先保存越南语选择。工作区继续保持原 URL，不使用客户端语言跳转。
- [文本流式] 增加 Chat 等原生流的双层脱敏传输诊断：按 Attempt 记录帧序、字节数、最后上游帧/文本时间、`finishReason`、usage、终止事件与 `[DONE]`，并保留 `reader.read()` 的结构化根错误及 Abort/阶段超时来源；解析契约、失败状态、故障切换和计费语义保持不变。
- [文本流式] 修复显式重试复用已释放预留及残留上游状态；重试按独立计费周期保留历史审计，孤儿预留按所属周期恢复，避免重试成功后旧失败周期被重复扣费。流协议与计费共用完整 SSE 分帧，终态立即收尾，并保留失败响应的用量。
- [文本流式] 逻辑模型绑定能力档案新增可选 `streamingTimeouts` 阶段截止时间（连接、首字节、首段文本、空闲）；后台以秒配置并保存为正整数毫秒，空值只使用总请求超时，阶段值不得超过按同一策略解析的总超时。文本绑定的有效 `timeoutMs` 现与其他能力一样参与请求超时策略，未配置或无效时仍回退 3 分钟。
- [后台] 生成运维的既有文本任务详情现在展示已持久化的流式尝试、协议/传输方式、阶段耗时、Token 用量、里程碑和错误；这些内部诊断没有加入用户侧文本任务响应或 SSE。
- [品牌] 产品展示名、文档元数据和内置图标统一为 HOTX AI 与 `/hx-favicon.png`；移除默认友情链接与开源社区入口。
...
- [声音] 新增经 Dflop 的声音克隆、声音档案与试听缓存，Create、Agent、Canvas 和短剧统一使用安全的预置/个人音色选择；Provider Voice ID、渠道密钥、Trace 与源音频签名地址仅保留在服务端。
...
- [界面] 充值中心提供响应式预设网格与整数 VND 自定义金额，并通过服务端报价进入安全支付流程。
...
- [商业] 预付费积分充值与邀请奖励形成使用链路；支付系统支持渠道锁定、金额和币种校验、幂等回调、整单退款及对账，并提供 Stripe、支付宝、微信 Native 与 PayPly 协议适配。
- [后台] 管理后台按九个业务分组组织导航，并提供生成运维、公开账号 ID、作品与媒体管理、人工接管及移动端布局；积分充值入口统一使用 PAYG 报价与支付流程。
...
- [修复] 系统文本调用显式携带逻辑模型计费标识；工作台、统一 Agent 和自动复盘使用区分候选绑定的稳定幂等键，取消请求会中断服务端规划，规划成功但会话或 Run 保存失败会退款；0 积分文本模型的失败调用也记录并撤销幂等用量。
...
- [修复] 文本等模型扣费统一按逻辑模型 ID 命中后台单价，已有上游模型单价会按启用绑定优先级继续生效；前端预计积分与服务端实扣使用同一价格，后台可完整重置旧别名价格；配置为 0 积分的模型仍执行幂等与失败用量回滚。
...
- [文档] README 首屏改为公开首页截图；补充完整用户端、管理后台、平台能力与部署功能清单，以及仓库目录、关键文件职责、参与贡献指南、开源交流 QQ 群、赞助二维码和项目致谢；公开文案统一使用“画布”和“HOTX AI”，并移除旧 QQ、爱发电及未确认的 Telegram/X/Instagram 默认入口。
...
- [商业] 提供用户、统一积分、CDK、充值订单、支付渠道、退款、对账、财务流水、公告、提示词、生成运营和审计日志后台。

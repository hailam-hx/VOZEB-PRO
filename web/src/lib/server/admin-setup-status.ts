...
function buildAdminSetupSummary(input: { settings: AuthSettings; userSummary: PublicUserSummary; presets?: Array<{ enabled: boolean }>; paymentConfig: Awaited<ReturnType<typeof getPaymentConfigSummary>> }): AdminSetupSummary {
    ...
    const steps: AdminSetupStep[] = [
        ...
        },
        {
            ...
            description: paymentProviders.length > 0 ? "真实支付渠道已具备下单和回调接入条件。" : "当前可先用后台人工确认收款；正式运营前建议配置 Stripe、支付宝、微信支付、ZaloPay 或 PayPly。",
            href: "/admin?section=payments",
            actionLabel: "查看支付",
            accent: "amber",
            facts: [
                `真实渠道 ${paymentProviders.length} 个`,
                paymentProviders.length ? paymentProviders.join(" / ") : "Stripe / 支付宝 / 微信 / ZaloPay / PayPly 待配置",

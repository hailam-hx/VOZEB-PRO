...
import { subscribeTopUpOrderEvent } from "./top-up-order-event-signal";

describe("top-up order event signal", () => {
    it("filters PostgreSQL notifications by stable order ID", async () => {
        const listener = vi.fn();
        const unsubscribe = await subscribeTopUpOrderEvent(" order-one ", listener);

        expect(mocks.subscribe).toHaveBeenCalledWith("vozeb_pro_top_up_order_events", expect.any(Function));

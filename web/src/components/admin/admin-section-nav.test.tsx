...
describe("admin navigation order", () => {
    it("keeps the existing business classifications and generation operations entry", () => {
        ...
        expect(adminSectionGroups.find((group) => group.title === "商品运营")?.items.map((item) => item.label)).toEqual(["充值与定价", "订单管理"]);
        expect(adminSectionGroups.find((group) => group.title === "营销推广")?.items.map((item) => item.label)).toEqual(["邀请奖励"]);
        expect(adminSectionGroups.find((group) => group.title === "财务管理")?.items.map((item) => item.label)).toEqual(["积分账务", "支付渠道", "CDK 兑换", "财务流水"]);

...
export const adminHelpArticles: AdminHelpArticle[] = [
    {
        ...
        category: "开始使用",
        title: "首次配置与上线顺序",
        summary: "从数据库初始化到模型、充值定价、支付和存储，按依赖顺序完成后台首次配置。",
        ...
        steps: [
            ...
            {
                title: "配置站点和模型能力",
                description: "先设置站点名称与 HOTX AI Logo，再配置上游渠道、逻辑模型和默认文本模型。",
                checks: ["前台、登录页和后台品牌一致", "用户端文本调用成功", "已设置默认文本逻辑模型"],
            },
            {
                title: "配置充值、定价和支付",
                description: "创建 VND 充值预设，核对逻辑售价与绑定成本，再按实际商户资质启用支付渠道。",
                checks: ["至少一个充值预设已启用", "逻辑售价与绑定成本有版本", "支付回调地址与站点 HTTPS 域名一致"],
            },
            {
                title: "确定存储和恢复方案",
        ...
        links: [
            ...
            { label: "充值与定价", description: "配置充值预设、汇率、售价与成本", section: "top-ups" },
    ...
    },
    {
        ...
        category: "经营分析",
        title: "经营看板、用户与生成运维",
        summary: "查看平台运行趋势，管理用户状态与积分，并定位生成任务和上游调用问题。",
        ...
        steps: [
            ...
            {
                title: "按用户核对权限和积分",
                description: "在用户运营中查看账号状态、角色和统一积分余额；人工调整前先确认业务原因。",
                checks: ["未误改管理员角色", "积分调整有明确原因", "禁用用户无法继续创建任务"],
            },
            {
                title: "从调用记录定位单次请求",
    ...
    },
    {
        ...
        category: "充值运营",
        title: "充值预设、定价、邀请与订单",
        summary: "配置客户汇率、充值预设、逻辑售价和绑定成本，并以不可变订单快照完成收款和售后。",
        keywords: ["充值", "预设", "越南盾", "客户汇率", "逻辑售价", "绑定成本", "上游单位", "邀请", "订单", "退款", "恢复", "对账"],
        purpose: "把用户支付、积分到账、模型销售收入和真实上游成本连成可审计链路，并确保历史订单不受后续汇率或定价版本修改影响。",
        steps: [
            {
                title: "配置客户汇率与充值预设",
                description: "设置 VND 对积分的客户汇率版本，并创建可启停、可排序的整数越南盾充值预设。",
                checks: ["金额为正整数 VND", "积分和优惠由服务器报价", "旧订单保留创建时汇率版本"],
            },
            {
                title: "配置逻辑模型售价",
                description: "按逻辑模型设置用户侧积分售价；这是销售定价，不与任何单个上游供应商成本混用。",
                checks: ["售价版本明确", "零用量也有可审计记录", "实际结算读取订单或用量快照"],
            },
            {
                title: "配置绑定成本和单位换算",
                description: "为每个逻辑模型绑定保存供应商报价、单位换算和成本版本，用真实用量计算成本与毛利。",
                checks: ["供应商币种和计价单位明确", "单位换算可复核", "前台接口不暴露成本和毛利"],
            },
            {
                title: "配置邀请奖励",
                description: "设置邀请人积分、新用户积分、首充最低 USD 名义价值、冷静期、月度/活动上限和风险冻结策略。",
                checks: ["积分奖励和门槛与获客成本预算一致", "首充使用已验证订单 USD 快照", "维护任务与人工结算入口可用"],
            },
            {
                title: "按订单快照处理售后",
                description: "核对原生支付金额、优惠或活动快照、USD 快照和积分发放，再执行退款。",
                checks: ["支付金额与订单快照一致", "退款状态和积分撤销可追踪", "支付商金额与本地金额可对账"],
            },
        ],
        checks: ["前台报价、支付页和订单详情金额一致", "用量、成本、毛利和异常记录可下钻", "孤儿冻结能够恢复或释放", "邀请注册、首单、待结算、已发放和撤销口径能够区分"],
        warnings: ["客户端只能提交充值预设或整数 VND 金额，禁止接受客户端积分、汇率、定价版本或成本字段。", "订单创建后只能使用订单保存的金额和规则快照，不能按当前配置重算。", "邀请奖励只支持单层关系；单一网络信号只能触发复核或冻结，不能直接认定欺诈。"],
        links: [
            { label: "充值与定价", description: "管理预设、客户汇率、逻辑售价与绑定成本", section: "top-ups" },
    ...
    },
    {
        ...
        purpose: "保证一次充值、生成、退款或兑换都能在订单、支付流水、统一积分流水和模型用量之间相互核对。",
        steps: [
            {
                title: "建立统一积分规则",
                description: "按逻辑模型配置基础积分，并设置图片数量、视频时长、清晰度等参数倍率。",
                checks: ["用户预计积分与服务端实际扣费使用同一规则", "零积分调用也会记录幂等用量", "失败任务能释放冻结积分"],
            },
            {
                title: "配置并检测支付渠道",
                description: "只启用已经具备商户资质、密钥、证书和 HTTPS 回调的渠道，逐项完成测试支付。",
                checks: ["回调验签通过", "重复和乱序事件不重复入账积分", "退款能够回写订单和流水"],
            },
            {
                title: "发行和追踪 CDK",
                description: "按活动或售后场景生成积分兑换码，设置次数、有效期和备注，明文仅在生成当次导出。",
                checks: ["发放范围和数量可追踪", "过期或停用码不可兑换", "兑换用户和时间可核对"],
            },
            {
                title: "每日核对财务流水",
                description: "对比订单、支付渠道实收、退款、积分负债和异常对账项，发现差异后先保留证据再处理。",
                checks: ["已支付订单存在成功支付流水", "退款订单的退款流水和积分冲正一致", "人工调整有备注和责任人"],
            },
        ],
    ...
    },
    {
        ...
        steps: [
            {
                title: "维护站点资料和品牌",
                description: "设置站点名称、SEO、社交入口、首页展示和 HOTX AI Logo；浏览器图标与助手头像使用同一品牌来源。",
        ...
        checks: ["站点所有品牌入口使用 HOTX AI Logo", "注册、验证码和找回密码流程与 SMTP 状态一致", "注销和敏感操作都有可追踪记录"],
    ...
    },
    {
        ...
        steps: [
            ...
            },
            {
                ...
                checks: ["上游只创建了一次任务", "轮询查询的是同一个任务 ID", "失败释放和积分退款正确"],
            },
            {
                title: "检查账务和存储",
                description: "生成故障核对积分流水，支付故障核对订单与支付流水，媒体故障核对 storage_provider、对象或本地文件。",
                checks: ["没有重复扣费或积分入账", "退款不依赖页面再次打开", "媒体 URL 没有泄露对象 Key 或永久上游地址"],
            },
            {
                title: "恢复后复盘",

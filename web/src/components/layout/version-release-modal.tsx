"use client";

import type { CSSProperties, ReactNode } from "react";
import { Modal, Tag, Timeline } from "antd";
import { useVersionCheck } from "@/hooks/use-version-check";
import { APP_VERSION } from "@/constant/env";
import { useTranslations } from "next-intl";

const releaseTagColors: Record<string, string> = {
    Agent: "geekblue",
    生成: "green",
    上游: "cyan",
    创作: "magenta",
    "Canvas/短剧": "purple",
    作品: "gold",
    商业: "volcano",
    后台: "blue",
    界面: "lime",
    安全: "red",
    性能: "cyan",
    工程: "geekblue",
    安装: "orange",
    发布: "orange",
    "品牌/文档": "purple",
    新增: "green",
    修复: "red",
    优化: "cyan",
    调整: "blue",
    二开: "gold",
    致谢: "volcano",
    文档: "purple",
};

export function getReleaseTagColor(type: string) {
    return releaseTagColors[type] || "default";
}

type VersionReleaseModalProps = {
    className?: string;
    style?: CSSProperties;
    label?: ReactNode;
};

export function VersionReleaseModal({ className, style, label }: VersionReleaseModalProps) {
    const t = useTranslations("common.version");
    const { open, setOpen, openReleaseModal, latestVersion, releases, checking, hasNewVersion, checkLatestRelease } = useVersionCheck();

    return (
        <>
            <button
                type="button"
                className={className || "shrink-0 cursor-pointer text-xs font-medium text-stone-500 transition hover:text-stone-950 dark:text-stone-400 dark:hover:text-white"}
                style={style}
                onClick={openReleaseModal}
                title={t("viewUpdates")}
            >
                {label || (
                    <span className="relative inline-flex">
                        {APP_VERSION}
                        {hasNewVersion ? <span className="absolute -right-1.5 -top-1 size-1.5 rounded-full bg-green-500" /> : null}
                    </span>
                )}
            </button>
            <Modal title={t("title")} open={open} width={680} centered footer={null} onCancel={() => setOpen(false)}>
                <div className="mb-5 grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="text-xs text-stone-500 dark:text-stone-400">{t("currentVersion")}</div>
                        <div className="mt-1 text-base font-semibold text-stone-950 dark:text-stone-100">{APP_VERSION}</div>
                    </div>
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="flex items-center justify-between gap-3">
                            <div className="text-xs text-stone-500 dark:text-stone-400">{t("latestVersion")}</div>
                            <button
                                type="button"
                                className="cursor-pointer bg-transparent p-0 text-[11px] font-normal text-stone-400 underline-offset-2 transition hover:text-stone-700 hover:underline dark:text-stone-500 dark:hover:text-stone-300"
                                onClick={() => void checkLatestRelease(true)}
                            >
                                {checking ? t("checking") : t("checkUpdates")}
                            </button>
                        </div>
                        <div className="mt-1 text-base font-semibold text-stone-950 dark:text-stone-100">{latestVersion}</div>
                    </div>
                </div>
                <div className="max-h-[56vh] overflow-y-auto pr-2">
                    <Timeline
                        items={releases.map((release) => ({
                            content: (
                                <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        <span className="text-sm font-semibold text-stone-950 dark:text-stone-100">{release.version === "Unreleased" ? t("unreleased") : release.version}</span>
                                        <span className="text-xs text-stone-500 dark:text-stone-400">{release.date}</span>
                                        <div className="flex min-w-0 items-center gap-1.5">
                                            {release.version === latestVersion ? <Tag color="green">{t("latest")}</Tag> : null}
                                            {release.version === APP_VERSION ? <Tag>{t("current")}</Tag> : null}
                                        </div>
                                    </div>
                                    <div className="mt-2 space-y-1.5">
                                        {release.items.map((item, index) => (
                                            <div key={`${release.version}-${index}`} className="flex items-start gap-2 text-sm leading-6 text-stone-700 dark:text-stone-300">
                                                <Tag color={getReleaseTagColor(item.type)} className="m-0 mt-0.5 shrink-0 whitespace-nowrap">
                                                    {releaseTagLabel(item.type, t)}
                                                </Tag>
                                                <span className="min-w-0 flex-1">{item.content}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ),
                        }))}
                    />
                </div>
            </Modal>
        </>
    );
}

function releaseTagLabel(type: string, t: ReturnType<typeof useTranslations>) {
    const key = releaseTagKeys[type];
    return key ? t(`tags.${key}`) : type;
}

const releaseTagKeys: Record<string, string> = {
    Agent: "agent",
    生成: "generation",
    上游: "upstream",
    创作: "creation",
    "Canvas/短剧": "canvasDrama",
    作品: "works",
    商业: "commerce",
    后台: "admin",
    界面: "interface",
    安全: "security",
    性能: "performance",
    工程: "engineering",
    安装: "installation",
    发布: "release",
    "品牌/文档": "brandDocs",
    新增: "added",
    修复: "fixed",
    优化: "optimized",
    调整: "changed",
    二开: "customization",
    致谢: "thanks",
    文档: "docs",
};

...
export function CommunityActivityModal({ view, onClose, onChanged, onOpenCreator, onOpenWork }: { view?: CommunityActivityView; onClose: () => void; onChanged: () => void; onOpenCreator: (username: string) => void; onOpenWork: (slug: string) => void }) {
    const t = useTranslations("public.communityActivity");
    ...
    const unfollow = async (item: CommunityUser) => {
        ...
        try {
            ...
            message.error(t("unfollowFailed"));
        } finally {
            setActionKey("");
        }
    };
    ...
    return (
        <Modal open={Boolean(view)} title={view ? t(`${view}.title`) : t("title")} width={640} centered footer={null} destroyOnHidden onCancel={onClose} styles={{ body: { maxHeight: "min(64vh, 620px)", overflowY: "auto", padding: "12px 0 0" } }}>
            <div className="min-h-40">
                ...
                {!loading && view && activityPage?.view === view && !activityPage.items.length ? <CompactEmptyState title={t(`${view}.emptyTitle`)} description={t(`${view}.emptyDescription`)} /> : null}
            </div>
                ...
                const identity = (
                    <>
                        ...
                        <span className="min-w-0 flex-1">
                            ...
                            <span className="mt-0.5 block truncate text-xs text-muted-foreground">{t("followerCount", { username: item.username, count: item.followerCount })}</span>
                ...
                return (
                    <div key={item.username} className="flex min-w-0 items-center gap-2 py-3 first:pt-0 last:pb-0 sm:gap-3">
                        ...
                        <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
                            <time className="hidden text-[10px] text-muted-foreground md:block" title={relation === "following" ? t("followedAt") : t("becameFollowerAt")}>
                                {format.dateTime(new Date(item.relatedAt), { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}
                            </time>
                            {relation === "following" ? (
                ...
                <div key={item.slug} className="flex min-w-0 items-center gap-2 py-3 first:pt-0 last:pb-0">
                    <button type="button" className="flex min-w-0 flex-1 items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => onOpen(item.slug)}>
                        ...
                        <span className="min-w-0 flex-1">
                            ...
                            <span className="mt-1 block truncate text-xs text-muted-foreground">{item.authorName || t("anonymous")}</span>
                            <span className="mt-1 block text-[10px] text-muted-foreground">{t("likedAt", { time: format.dateTime(new Date(item.likedAt), { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }) })}</span>
                        </span>
                    </button>

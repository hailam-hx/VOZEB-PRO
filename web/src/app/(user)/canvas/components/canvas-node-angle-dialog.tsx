    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={860} centered destroyOnHidden>
            <div className="space-y-5">
                <div>
                    <h2 className="text-xl font-semibold">{t("dialogs.angle.title")}</h2>
                    <p className="mt-1 text-sm opacity-60">{t("dialogs.angle.description")}</p>
                </div>
                <div className="grid gap-4 md:grid-cols-[minmax(260px,1fr)_360px] md:gap-6">
                    ...
                    <div className="space-y-4 py-2 md:space-y-6">
                        <AngleSlider label={t("dialogs.angle.horizontal")} value={params.horizontalAngle} min={-60} max={60} step={1} suffix="deg" onChange={(value) => update("horizontalAngle", value)} />
                        <AngleSlider label={t("dialogs.angle.pitch")} value={params.pitchAngle} min={-45} max={45} step={1} suffix="deg" onChange={(value) => update("pitchAngle", value)} />
                        <AngleSlider label={t("dialogs.angle.distance")} value={params.cameraDistance} min={1} max={10} step={0.1} onChange={(value) => update("cameraDistance", value)} />
                        <div className="grid grid-cols-[88px_1fr_72px] items-center gap-4">
                            <span className="font-medium opacity-75">{t("dialogs.angle.wideLens")}</span>
                            <Segmented
                                className="w-fit"
                                value={params.wideAngle ? "wide" : "standard"}
                                options={[
                                    { label: t("dialogs.angle.standard"), value: "standard" },
                                    { label: t("dialogs.angle.wide"), value: "wide" },
                                ]}

...
export function CanvasNodeCropDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (crop: CanvasImageCropRect) => void }) {
    ...
    return (
        <Modal title={t("dialogs.crop.title")} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={780} centered destroyOnHidden>
            <div className="space-y-4">
                        ...
                        <div className="absolute cursor-move border-2 border-white shadow-[0_0_0_1px_rgba(0,0,0,.3),0_0_28px_rgba(0,0,0,.28)]" style={cropStyle(crop)} onPointerDown={(event) => startDrag("move", event)}>
                            ...
                            {handles.map((handle) => (
                                <button
                                    ...
                                    aria-label={t("dialogs.crop.resizeFrame")}
                                />
                            ))}
                        </div>
                    </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-2">
                    <div className="flex flex-wrap items-center gap-3 text-sm opacity-80">
                        <span>{t("dialogs.crop.cropSize", { value: cropSize ? `${cropSize.width} x ${cropSize.height}` : t("dialogs.unknown") })}</span>
                        <span>{t("dialogs.crop.ratio", { value: cropSize ? formatRatio(cropSize.width, cropSize.height) : t("dialogs.unknown") })}</span>
                        {image ? <span>{t("dialogs.crop.originalDimensions", { width: image.width, height: image.height })}</span> : null}
                    </div>
                    <Button icon={locked ? <Lock className="size-4" /> : <LockOpen className="size-4" />} onClick={() => setLocked((value) => !value)}>
                        {locked ? t("dialogs.crop.lockedRatio") : t("dialogs.crop.freeRatio")}
                    </Button>
                </div>

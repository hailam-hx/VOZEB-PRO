...
export function CanvasTopBar({
    ...
    return (
        <>
                ...
                <div className="canvas-topbar-left pointer-events-auto flex min-w-0 items-center gap-2 sm:gap-3">
                    <Dropdown
                        ...
                        menu={{
                            ...
                            items: [
                                { key: "workbench", icon: <Sparkles className="size-4" />, label: t("workbench"), onClick: onWorkbench },
                                { key: "docs", icon: <BookOpen className="size-4" />, label: t("help"), onClick: () => window.location.assign("/help?section=canvas") },
                                { type: "divider" },
                                { key: "import", icon: <Upload className="size-4" />, label: t("importMedia"), onClick: onImportImage },
                                { type: "divider" },
                                { key: "undo", disabled: !canUndo, icon: <Undo2 className="size-4" />, label: <MenuLabel text={t("undo")} shortcut="⌘ Z" />, onClick: onUndo },
                                { key: "redo", disabled: !canRedo, icon: <Redo2 className="size-4" />, label: <MenuLabel text={t("redo")} shortcut="⌘ ⇧ Z / ⌘ Y" />, onClick: onRedo },
                                { type: "divider" },
                                { key: "delete", danger: true, icon: <Trash2 className="size-4" />, label: t("deleteCanvas"), onClick: onDeleteProject },
                            ],
                        }}
                    >
                        <button ref={menuTriggerRef} type="button" className="grid size-9 place-items-center rounded-full transition hover:bg-black/5 dark:hover:bg-white/10" style={{ color: theme.node.text }} aria-label={t("openCanvasMenu")}>
                    ...
                    <button
                        ...
                        aria-label={assetsOpen ? t("closeAssetsPanel") : t("openAssetsPanel")}
            ...
            <Modal title={t("shortcuts")} open={shortcutsOpen} onCancel={() => setShortcutsOpen(false)} footer={null} centered width={640}>
                <div className="max-h-[min(72vh,620px)] space-y-0.5 overflow-y-auto border-t pt-3 text-sm" style={{ borderColor: theme.node.stroke }}>
                    <Shortcut keys={[t("dragCanvas")]} value={t("panView")} theme={theme} />
                    <Shortcut keys={[t("mouseWheel")]} value={t("zoomCanvas")} theme={theme} />
                    <Shortcut keys={[t("zoomSlider")]} value={t("preciseZoom")} theme={theme} />
                    <Shortcut keys={["Ctrl / Cmd", t("drag")]} value={t("boxSelectNodes")} theme={theme} />
                    <Shortcut keys={["Shift / Ctrl / Cmd", t("click")]} value={t("addNodeSelection")} theme={theme} />
                    <Shortcut keys={["Ctrl / Cmd", "A"]} value={t("selectAllNodes")} theme={theme} />
                    <Shortcut keys={["Ctrl / Cmd", "C / V"]} value={t("copyPasteNodes")} theme={theme} />
                    <Shortcut keys={["Ctrl / Cmd", "Z"]} value={t("undo")} theme={theme} />
                    <Shortcut keys={["Ctrl / Cmd", "Shift", "Z"]} value={t("redo")} theme={theme} />
                    <Shortcut keys={["Ctrl / Cmd", "Y"]} value={t("redo")} theme={theme} />
                    <Shortcut keys={["Delete / Backspace"]} value={t("deleteSelected")} theme={theme} />
                    <Shortcut keys={["Esc"]} value={t("cancelSelectionAndClose")} theme={theme} />
                    <Shortcut keys={[t("dragMediaIn")]} value={t("uploadToCanvas")} theme={theme} />
                </div>
            </Modal>
        </>
    );
...
function CompactAgentStatus({ status, onClick }: { status: { connected: boolean; enabled: boolean; activity: string }; onClick: () => void }) {
    ...
    const colorTheme = useThemeStore((state) => state.theme);
    const theme = canvasThemes[colorTheme];
    const label = status.connected ? t("connectedToLocalCodex") : status.enabled ? t("connecting") : t("connectingToLocalCodex");

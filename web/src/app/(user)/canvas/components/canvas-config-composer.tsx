...
export function CanvasConfigComposer({ value, inputs, onChange, onClose }: CanvasConfigComposerProps) {
    const t = useTranslations("canvas.configComposer");
        ...
        >
            <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-baseline gap-2">
                    <div className="shrink-0 text-xs font-semibold">{t("title")}</div>
                    <div className="truncate text-[11px] opacity-55">{t("description")}</div>
                </div>
                <Button aria-label={t("close")} size="small" type="text" className="!h-7 !w-7 !min-w-7 !p-0" icon={<X className="size-3.5" />} onClick={onClose} />
            </div>
            <div className="relative rounded-xl border" style={{ background: theme.node.fill, borderColor: theme.node.stroke }}>
                ...
                />
                {mention && candidates.length ? <MentionMenu inputs={candidates} activeIndex={Math.min(activeIndex, candidates.length - 1)} theme={theme} onSelect={insertReference} labelFor={(input) => resourceLabel(input, inputs, t)} /> : null}
            </div>
            {imagePreview ? (
                <Image src={imagePreviewUrl(imagePreview, 1920)} alt={t("imagePreview")} style={{ display: "none" }} preview={{ open: true, src: imagePreviewUrl(imagePreview, 1920), onOpenChange: (open) => !open && setImagePreview(null) }} />
            ) : null}
        </div>
    );
}
...
function MentionMenu({
    ...
    labelFor: (input: NodeGenerationInput) => string;
}) {
            ...
            {inputs.map((input, index) => (
                ...
                >
                    ...
                    <span className="min-w-0 flex-1">
                        <span className="block font-medium">{labelFor(input)}</span>
...
function resourceLabel(input: NodeGenerationInput, inputs: NodeGenerationInput[], t: (key: `resources.${NodeGenerationInput["type"]}`, values: { index: number }) => string) {
    const sameTypeInputs = inputs.filter((item) => item.type === input.type);
    const index = Math.max(
        0,
        sameTypeInputs.findIndex((item) => item.nodeId === input.nodeId),
    );
    return t(`resources.${input.type}`, { index: index + 1 });
}

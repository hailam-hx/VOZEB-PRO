        ...
        >
            {activeAsset.type === "audio" ? (
                <figure className="col-start-1 row-start-1 w-[352px] max-w-full rounded-xl border border-[#e7e9ee] bg-white p-4 dark:border-[#303640] dark:bg-[#181b20]">
                    <AgentMediaPreview type="audio" url={url} title={activeAsset.title || t("generatedAudio")} />
                </figure>
            ) : (
                <figure
                ...
                >
                    <AgentMediaPreview
                        ...
                        url={url}
                        title={activeAsset.title || t(activeAsset.type === "video" ? "generatedVideo" : "generatedImage")}
                        className="size-full"
                        fit="contain"
            ...
            <CreativeResultSwitcher
                ...
                renderThumbnail={(asset, index) => {
                    ...
                    if (asset.type === "image") return <img src={imagePreviewUrl(previewUrl, 240)} alt={asset.title || t("generatedResultNumber", { number: index + 1 })} loading="lazy" className="size-full object-cover" />;
                    if (asset.type === "video") {
                        const coverUrl = typeof asset.metadata.coverUrl === "string" ? asset.metadata.coverUrl.trim() : "";
                        return coverUrl ? (
                            <img src={imagePreviewUrl(coverUrl, 240)} alt={asset.title || t("generatedVideoNumber", { number: index + 1 })} loading="lazy" className="size-full object-cover" />
                        ) : (
                            <span className="grid size-full place-items-center text-[#667085] dark:text-[#a4adb8]" aria-label={asset.title || t("generatedVideoNumber", { number: index + 1 })}>

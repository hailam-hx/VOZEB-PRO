...
describe("generationPreferenceSummary", () => {
    it("keeps image, video and audio settings readable in one compact label", () => {
        ...
        expect(generationPreferenceSummary("audio", { audio: { voiceSelection: { type: "preset", voiceId: "nova" }, voiceName: "Nova", format: "wav", speed: 1.25 } }, zhCopy)).toBe("Nova · WAV · 1.25x");
        expect(generationPreferenceSummary("audio", { audio: { voiceSelection: { type: "preset", voiceId: "narrator-pro" }, format: "m4a" } }, zhCopy)).toBe("narrator-pro · m4a");

...
export function DramaAssetsPanel({ project, episode }: { project: DramaProject; episode: DramaEpisode }) {
    ...
    const definitionTitle = t(`kinds.${activeKind}.title`);
    const filterOptions: Array<{ value: DramaAssetFilter; label: string }> = ["all", "current-episode", "missing-reference", "incomplete", "used", "unused"].map((value) => ({ value: value as DramaAssetFilter, label: t(`filters.${value}`) }));
    const sortLabels: Record<DramaAssetSort, string> = { default: t("sorts.default"), attention: t("sorts.attention"), usage: t("sorts.usage"), name: t("sorts.name") };
    ...
    const exportAssets = async () => {
        ...
        try {
            ...
            if (!result.exported) message.info(t("noDownloadableReferences"));
            else if (result.skipped) message.warning(t("downloadPartial", { exported: result.exported, skipped: result.skipped }));
            else message.success(t("downloadSuccess", { count: result.exported }));
        } catch {
    ...
    return (
        <div className="min-w-0" data-drama-assets-library>
            <div className="flex min-w-0 flex-col gap-2 border-b border-border pb-3 xl:flex-row xl:items-center" data-drama-assets-toolbar>
                <nav className="hide-scrollbar flex min-w-0 shrink-0 gap-1 overflow-x-auto" aria-label={t("categoriesAria")}>
                    {assetKinds.map((kind) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span style={active ? { color: "var(--background)" } : undefined}>{t(`kinds.${kind}.title`)}</span>
                ...
                <div className="flex min-w-0 flex-1 items-center gap-2 xl:justify-end">
                    <Input
                        ...
                        placeholder={t("search", { kind: definitionTitle })}
                        aria-label={t("search", { kind: definitionTitle })}
                    />
                    <div className="w-[116px] shrink-0">
                        <Select className="w-full" value={filter} options={filterOptions} onChange={setFilter} aria-label={t("filterAria")} />
                    </div>
                    <Dropdown
                        trigger={["click"]}
                        menu={{
                    ...
                    >
                        <Tooltip title={t("sort", { sort: sortLabels[sort] })}>
                            <Button className="!size-8 !min-w-8" icon={<ArrowDownUp className="size-3.5" />} aria-label={t("sort", { sort: sortLabels[sort] })} />
                        </Tooltip>
                    </Dropdown>
                    <Tooltip title={t("downloadReferences")}>
                        <Button
                            ...
                            aria-label={t("downloadReferences")}
                        />
                    </Tooltip>
                    <Button type="primary" className="!h-8 !shrink-0 !px-2.5" icon={<Plus className="size-3.5" />} onClick={() => setEditor({ kind: activeKind })} aria-label={t("newKind", { kind: definitionTitle })}>
                        <span className="hidden sm:inline">{t("newKind", { kind: definitionTitle })}</span>
                    </Button>
                </div>
            </div>
            ...
            {visibleRows.length ? (
                    ...
                    {visibleRows.map((row) => (
                        <DramaAssetCard
                            ...
                            onDelete={() => {
                                ...
                                message.success(t("kindDeleted", { kind: definitionTitle }));
                            }}
                        />
                    ))}
                </div>
                ...
                <div className="mt-3 flex min-h-20 min-w-0 items-center justify-between gap-3 rounded-md border border-dashed border-border bg-muted/20 px-3 py-3 sm:px-4" data-drama-assets-empty>
                    <div className="min-w-0">
                        <div className="text-sm font-medium">{hasActiveFilter ? t("noMatches", { kind: definitionTitle }) : t("emptyKind", { kind: definitionTitle })}</div>
                        <div className="mt-0.5 text-xs text-muted-foreground">{hasActiveFilter ? t("clearToViewAll") : t("reuseDescription")}</div>
                    </div>
                    {hasActiveFilter ? (
...
function DramaAssetCard({ kind, row, onEdit, onDelete }: { kind: DramaAssetKind; row: DramaAssetLibraryRow; onEdit: () => void; onDelete: () => void }) {
    ...
    return (
        <article className="group relative min-w-0 overflow-hidden rounded-md border border-border bg-card transition hover:border-foreground/25 hover:shadow-[0_6px_18px_rgba(15,23,42,.07)]">
            <button type="button" className="block w-full text-left" onClick={onEdit} aria-label={t("editNamed", { kind: kindTitle, name: asset.name })}>
                <div className="grid aspect-[16/10] w-full place-items-center overflow-hidden bg-muted/55">
                    {primary?.url ? (
                        <Image src={imagePreviewUrl(primary.url, 480)} alt={t("referenceAlt", { name: asset.name })} rootClassName="!block !size-full" className="!size-full !object-cover transition duration-300 group-hover:scale-[1.02]" preview={false} />
                    ) : (
                        <div className="grid gap-1.5 text-center text-muted-foreground">
                            <ImagePlus className="mx-auto size-5" aria-hidden />
                            <span className="text-[11px]">{t("missingReference")}</span>
                        </div>
                    )}
                </div>
                <div className="min-w-0 px-2.5 py-2">
                    <div className="flex min-w-0 items-center gap-2 pr-7">
                        ...
                        {!primary ? <span className="shrink-0 text-[10px] font-medium text-amber-600 dark:text-amber-300">{t("missingReferenceShort")}</span> : null}
                    </div>
                    <p className="mt-1 truncate text-xs leading-4 text-muted-foreground" title={asset.description || undefined}>
                        {asset.description || t("noPurpose", { kind: kindLabel })}
                    </p>
                    <div className="mt-2 flex min-w-0 items-center gap-3 text-[11px] text-muted-foreground">
                        <Tooltip title={t("referenceCount", { count: referenceCount })}>
                            <span className="inline-flex items-center gap-1" aria-label={t("referenceCount", { count: referenceCount })}>
                                <Images className="size-3" aria-hidden />
                                <span className="tabular-nums">{referenceCount}</span>
                            </span>
                        </Tooltip>
                        <Tooltip title={t("usageCount", { count: usageCount })}>
                            <span className={`inline-flex items-center gap-1 ${usageCount ? "text-foreground/75" : ""}`} aria-label={t("usageCount", { count: usageCount })}>
                                <Video className="size-3" aria-hidden />
                                <span className="tabular-nums">{usageCount}</span>
                            </span>
                        </Tooltip>
                        {incomplete ? <span className="ml-auto shrink-0 text-amber-600 dark:text-amber-300">{t("incomplete")}</span> : usageCount === 0 ? <span className="ml-auto shrink-0">{t("unused")}</span> : null}
                    </div>
                </div>
            </button>
            <Popconfirm title={t("deleteNamedConfirm", { kind: kindTitle, name: asset.name })} description={t("deleteDescription")} okText={t("delete")} cancelText={t("cancel")} onConfirm={onDelete}>
                <Tooltip title={t("deleteKind", { kind: kindTitle })}>
                    <Button
                        type="text"
                        shape="circle"
                        ...
                        aria-label={t("deleteNamed", { kind: kindTitle, name: asset.name })}
                    />
                </Tooltip>
            </Popconfirm>
        </article>
    ...
    return (
        <section className="mt-4 overflow-hidden rounded-md border border-border bg-card/60" data-drama-source-assets>
            <button type="button" className="flex h-10 w-full items-center gap-2 px-3 text-left text-sm transition hover:bg-muted/60" onClick={() => setExpanded((current) => !current)} aria-expanded={expanded}>
                ...
                <span className="font-medium">{t("sourceAssets")}</span>
                <span className="text-xs tabular-nums text-muted-foreground">{project.sourceAssets?.length || 0}</span>
                <span className="ml-auto text-xs text-muted-foreground">{expanded ? t("collapse") : t("expand")}</span>
            </button>
            {expanded ? (
                        ...
                        return (
                            <article key={asset.id} className="flex min-w-0 items-center gap-2 overflow-hidden rounded-md border border-border bg-background p-1.5">
                                <div className="grid size-12 shrink-0 place-items-center overflow-hidden rounded bg-muted/55">
                                    {url && asset.type !== "text" ? <AgentMediaPreview type={asset.type} url={url} title={asset.title || t("projectSourceAsset")} className="size-full" /> : <FileText className="size-4 text-muted-foreground" />}
                                </div>
                                <div className="min-w-0">
                                    <div className="truncate text-xs font-medium" title={asset.title}>
                                        {asset.title || t("untitledAsset")}
                                    </div>

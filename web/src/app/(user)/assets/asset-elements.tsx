...
export function AssetCard({
    ...
    return (
        <article className="group min-w-0 overflow-hidden rounded-xl border border-border bg-card transition-[border-color,transform] duration-200 hover:-translate-y-0.5 hover:border-foreground/30">
            <button type="button" aria-label={t("viewNamed", { title: asset.title })} className="relative block w-full overflow-hidden bg-muted text-left outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring" onClick={onOpen}>
                {cover ? (
                ...
                <span className="absolute bottom-2.5 left-2.5 inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-black/65 px-2.5 py-1 text-[11px] font-medium text-white backdrop-blur-sm">
                    {assetKindIcon(asset.kind)} {t(`kinds.${asset.kind}`)}
                </span>
            </button>
            <div className="min-w-0 p-3 sm:p-3.5">
                ...
                <p className="mt-1 truncate text-[11px] text-muted-foreground">{asset.source || t(`kinds.${asset.kind}`)}</p>
                    ...
                    <div className="flex shrink-0 items-center gap-0.5">
                        {asset.kind !== "video" ? (
                            <Tooltip title={t("editAction")}>
                                <Button type="text" size="small" shape="circle" icon={<PencilLine className="size-3.5" />} aria-label={action(t("editAction"))} onClick={onEdit} />
                            </Tooltip>
                        ) : null}
                        {asset.kind === "text" ? (
                            <Tooltip title={t("copyAction")}>
                                <Button type="text" size="small" shape="circle" icon={<Copy className="size-3.5" />} aria-label={action(t("copyAction"))} onClick={() => void onCopy(asset)} />
                            </Tooltip>
                        ) : (
                            <Tooltip title={t("downloadAction")}>
                                <Button type="text" size="small" shape="circle" icon={<Download className="size-3.5" />} aria-label={action(t("downloadAction"))} onClick={() => onDownload(asset)} />
                            </Tooltip>
                        )}
                        {onPublish ? (
                            <Tooltip title={t("publishAction")}>
                                <Button type="text" size="small" shape="circle" icon={<Share2 className="size-3.5" />} onClick={onPublish} aria-label={action(t("publishAction"))} />
                            </Tooltip>
                        ) : null}
...
export function AssetPreviewModal({ asset, onClose, onCopy, onDownload }: { asset: Asset | null; onClose: () => void; onCopy: (asset: Asset) => void; onDownload: (asset: Asset) => void }) {
    ...
    return (
        <Modal title={t("details")} open={Boolean(asset)} width={760} centered footer={null} onCancel={onClose} destroyOnHidden>
            {asset ? (
                    ...
                    <Space onClick={(event) => event.stopPropagation()}>
                        {asset.kind === "text" ? (
                            <Tooltip title={t("copyText")}>
                                <Button type="primary" shape="circle" icon={<Copy className="size-4" />} aria-label={t("copyNamed", { title: asset.title })} onClick={() => onCopy(asset)} />
                            </Tooltip>
                        ) : null}
                        {asset.kind !== "text" ? (
                            <Tooltip title={t("downloadKind", { kind: t(`kinds.${asset.kind}`) })}>
                                <Button type="primary" shape="circle" icon={<Download className="size-4" />} aria-label={t("downloadNamed", { title: asset.title })} onClick={() => onDownload(asset)} />
                            </Tooltip>
                        ) : null}
                    </Space>
                </div>
...
export function assetSummary(asset: Asset, unknownDuration = "--:--") {
    if (asset.kind === "text") return asset.data.content;
    if (asset.kind === "audio") return `${formatDuration(asset.data.durationMs, unknownDuration)} · ${formatBytes(asset.data.bytes)} · ${asset.data.mimeType}`;
...
function formatDuration(durationMs: number | undefined, unknownDuration: string) {
    if (!durationMs) return unknownDuration;

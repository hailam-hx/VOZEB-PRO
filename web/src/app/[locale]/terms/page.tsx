...
const highlightIcons = [UserRoundCheck, Coins, Ban] as const;
const sectionParagraphCounts = [2, 2, 3, 2, 3, 2, 2, 3] as const;

export async function generateMetadata({ params }: LocalizedPageProps): Promise<Metadata> {
    const locale = requireSeoPageLocale("terms", (await params).locale);
    const [site, t] = await Promise.all([getPublicSiteSettings(), getTranslations({ locale, namespace: "public.terms" })]);
...
export default async function TermsPage({ params }: LocalizedPageProps) {
    ...
    const sections = sectionParagraphCounts.map((count, index) => ({ title: t(`sections.${index}.title`), paragraphs: Array.from({ length: count }, (_, paragraph) => t(`sections.${index}.paragraphs.${paragraph}`, { site: site.title })) }));
    return (
            ...
            <script
                ...
                dangerouslySetInnerHTML={{
                    __html: serializeStructuredData({
                        ...
                        "@type": "WebPage",
                        url: new URL(getSeoPagePath("terms", locale)!, siteMetadataBase()).toString(),
                        name: t("title"),
                        description: t("metadataDescription", { site: site.title }),
                        inLanguage: locale,
                    }),

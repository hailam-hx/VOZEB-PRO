...
const mocks = vi.hoisted(() => ({
    ...
    browserIconHref: vi.fn((site: { iconUrl?: string; logoUrl?: string }) => site.iconUrl || site.logoUrl || "/hx-favicon.png"),
...
describe("site metadata routes", () => {
    ...
    it("keeps a site-local icon relative to the current host", async () => {
        mocks.getPublicSiteSettings.mockResolvedValue({ title: "默认站点", iconUrl: "/hx-favicon.png", logoUrl: "/hx-favicon.png" });

        const response = await favicon(new Request("http://localhost:3000/api/site-icon"));

        expect(response.headers.get("location")).toBe("/hx-favicon.png");
    });

    it("falls back to the infinite-evolution icon when favicon points back to itself", async () => {
        mocks.getPublicSiteSettings.mockResolvedValue({ title: "默认站点", iconUrl: "/favicon.ico", logoUrl: "/hx-favicon.png" });

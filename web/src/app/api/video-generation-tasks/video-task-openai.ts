...
export async function buildOpenAiVideoFormData(input: OpenAiVideoFormInput) {
    ...
    if (input.seconds !== undefined) formData.set("seconds", String(input.seconds));
    if (input.width !== undefined && input.height !== undefined) formData.set("size", `${input.width}x${input.height}`);

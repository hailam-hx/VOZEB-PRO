...
export async function PATCH(request: Request) {
    ...
    try {
        ...
        await safeRecordAuditLog({ action: "admin.top_up_pricing.save", actor: auditActorFromRequest(request, user), target: { type: "top_up_pricing", id: config?.pricingVersion || "current" }, metadata: { customerFxVersion: config?.customerFxVersion } });
